...
ShowHelpShortcuts() {
    try {
        ...
        leftColumnText .= "mypass2→Copy Password 2 to Clipboard`n`n"
...
TrayTip("Complete Manager Ready - UPDATED VERSION",
    ...
    "✓ mypass→Copy Password 1 to Clipboard`n" .
    "✓ mypass2→Copy Password 2 to Clipboard`n" .
    "✓ unzipit→PowerShell with 'ext' command`n" .
    "✓ updater→Launch Software Updater`n" .
    "✓ downloads→Open Downloads Folder in Explorer`n" .
    "✓ myasus→Open ASUS ROG Website in Chrome`n`n" .
    "••• UPDATED FEATURES •••`n" .
    ...
    "✓ Changed: reboot→rebootit | todo→mytodo`n" .
    "✓ Removed: recovery, parsec, qbittorrent as requested`n`n" .
    ...
    "✓ UnzipIt: Opens PowerShell and runs 'ext' command`n`n" .
    "••• HOTKEYS •••`n" .
    ...
    "mymail→Email Copy | mypass→Password 1 Copy | mypass2→Password 2 Copy`n" .
    "unzipit→PowerShell ext | updater→Software Updater`n" .
    ...
    "All shortcuts work perfectly with smart management and clipboard integration!", 30000)

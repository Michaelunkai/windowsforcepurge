...
textShortcuts := Map(
    ...
    "bramble", "BRAMBLE_GAME",  ; <-- NEW: Bramble game shortcut
    "1000x", "1000XRESIST_GAME",  ; <-- 1000xRESIST
    "60sec", "60SECONDSREATOMIZED_GAME",  ; <-- 60SecondsReatomized
    "advan", "ADVANCEDWARS_GAME",  ; <-- advancedwars
    "aithe", "AITHESOMNIUMFILES_GAME",  ; <-- AITHESOMNIUMFILES
    "bombr", "BOMBRUSHCYBERFUNK_GAME",  ; <-- bombrushcyberfunk
    "brigh", "BRIGHTMEMORYINFINITE_GAME",  ; <-- BrightMemoryInfinite
    "casse", "CASSETTEBEASTS_GAME",  ; <-- CassetteBeasts
    "deadr", "DEADRISING4_GAME",  ; <-- DeadRising4
    "duckg", "DUCKGAME_GAME",  ; <-- DuckGame
    "eastw", "EASTWARD_GAME",  ; <-- eastward
    "elden", "ELDENRINGNIGHTREIGN_GAME",  ; <-- ELDENRINGNIGHTREIGN
    "engar", "ENGARDE_GAME",  ; <-- EnGarde
    "enshr", "ENSHROUDED_GAME",  ; <-- Enshrouded
    "fullm", "FULLMETALFURIES_GAME",  ; <-- FullMetalFuries
    "godof", "GODOFWARRAGNAROK_GAME",  ; <-- God Of War Ragnarok
    "taint", "TAINTEDGRAIL_GAME",  ; <-- taintedGrailTheFallofAvalon
    "midni", "THEMIDNIGHTWALK_GAME"  ; <-- TheMidnightWalk
)
    ...
    for shortcut, command in textShortcuts {
        if (InStr(StrLower(textBuffer), StrLower(shortcut)) > 0) {
            ...
            if (command = "WALLPAPER_APP") {
                ...
                ForceKillActiveApp()
            } else if (command = "BRAMBLE_GAME") {
                LaunchBrambleGame()
            } else if (command = "1000XRESIST_GAME") {
                LaunchGame("1000xRESIST.lnk", "1000xRESIST")
            } else if (command = "60SECONDSREATOMIZED_GAME") {
                LaunchGame("60SecondsReatomized.lnk", "60SecondsReatomized")
            } else if (command = "ADVANCEDWARS_GAME") {
                LaunchGame("advancedwars.lnk", "Advanced Wars")
            } else if (command = "AITHESOMNIUMFILES_GAME") {
                LaunchGame("AITHESOMNIUMFILES.lnk", "AI: The Somnium Files")
            } else if (command = "BOMBRUSHCYBERFUNK_GAME") {
                LaunchGame("bombrushcyberfunk.lnk", "Bomb Rush Cyberfunk")
            } else if (command = "BRIGHTMEMORYINFINITE_GAME") {
                LaunchGame("BrightMemoryInfinite.lnk", "Bright Memory Infinite")
            } else if (command = "CASSETTEBEASTS_GAME") {
                LaunchGame("CassetteBeasts.lnk", "Cassette Beasts")
            } else if (command = "DEADRISING4_GAME") {
                LaunchGame("DeadRising4.lnk", "Dead Rising 4")
            } else if (command = "DUCKGAME_GAME") {
                LaunchGame("DuckGame.lnk", "Duck Game")
            } else if (command = "EASTWARD_GAME") {
                LaunchGame("eastward.lnk", "Eastward")
            } else if (command = "ELDENRINGNIGHTREIGN_GAME") {
                LaunchGame("ELDENRINGNIGHTREIGN.lnk", "Elden Ring Night Reign")
            } else if (command = "ENGARDE_GAME") {
                LaunchGame("EnGarde.lnk", "EnGarde")
            } else if (command = "ENSHROUDED_GAME") {
                LaunchGame("Enshrouded.lnk", "Enshrouded")
            } else if (command = "FULLMETALFURIES_GAME") {
                LaunchGame("FullMetalFuries.lnk", "Full Metal Furies")
            } else if (command = "GODOFWARRAGNAROK_GAME") {
                LaunchGame("God Of War Ragnarok REPACK KaOs.lnk", "God of War Ragnarok")
            } else if (command = "TAINTEDGRAIL_GAME") {
                LaunchGame("taintedGrailTheFallofAvalon.lnk", "Tainted Grail")
            } else if (command = "THEMIDNIGHTWALK_GAME") {
                LaunchGame("TheMidnightWalk.lnk", "The Midnight Walk")
            } else {
...
LaunchGame(shortcutName, gameDisplayName) {
    try {
        LogMessage("LAUNCHER: Attempting to launch " . gameDisplayName)
        gamePath := "C:\Users\misha\Desktop\games\" . shortcutName
        
        ; Check if game shortcut exists
        if !FileExist(gamePath) {
            LogMessage("ERROR: " . gameDisplayName . " shortcut not found at: " . gamePath)
            TrayTip(gameDisplayName . " Error", gameDisplayName . " shortcut not found at specified path", 3000)
            return
        }
        
        ; Launch game via shortcut
        Run('"' . gamePath . '"')
        LogMessage("LAUNCHER: " . gameDisplayName . " launched successfully from: " . gamePath)
        TrayTip(gameDisplayName, gameDisplayName . " launched successfully", 2000)
        
    } catch Error as e {
        LogMessage("ERROR in LaunchGame(" . gameDisplayName . "): " . e.message)
        TrayTip(gameDisplayName . " Critical Error", "Critical error: " . e.message, 3000)
        IncrementError()
    }
}

LaunchBrambleGame() {
    LaunchGame("bramble.lnk", "Bramble")
}

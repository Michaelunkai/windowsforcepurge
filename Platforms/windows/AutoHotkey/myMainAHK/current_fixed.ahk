...
OptimizeProcessResources(procName, restore := false) {
    try {
        if (!restore) {
            ...
            ; 2. Aggressive working set trimming (multiple passes)
            Loop 10 {
                TrimProcessWorkingSet(procName)
                Sleep(100)
            }
            LogMessage("OPTIMIZE: Aggressively trimmed working set for " . procName)
...
ShowSuspendedProcessesGUI() {
    try {
        ...
        ; Event handlers
        resumeBtn.OnEvent("Click", (*) => {
            selectedIndex := processList.Value
            if (selectedIndex > 0 && selectedIndex <= processArray.Length) {
                selectedProcess := processArray[selectedIndex]

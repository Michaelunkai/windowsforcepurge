...
OpenYoutube() {
    try {
        LogMessage("WEB: Opening YouTube in Chrome with automatic login")
        ...
        ; Open YouTube in Chrome with automatic login
        ; Using a profile that should have michaelovsky22@gmail.com logged in
        Run('"' . chromePath . '" --new-tab "https://youtube.com" --profile-directory="Profile 1"')
        LogMessage("WEB: YouTube opened in Chrome with automatic login profile")
        
        ; Wait a moment for the page to load
        Sleep(2000)
        
        ; Try to ensure we're logged in by checking for login elements
        ; If not logged in, we can attempt to auto-fill credentials
        try {
            ; Wait for YouTube to load and check if we need to login
            if WinWait("ahk_exe chrome.exe", , 5) {
                ; Send Tab to focus on the page
                Send("{Tab}")
                Sleep(500)
                
                ; Check if we're on the login page by looking for common login elements
                ; If we detect login page, we can auto-fill the email
                ; Note: This is a basic approach - YouTube's login flow may vary
                LogMessage("WEB: YouTube page loaded, checking login status")
            }
        } catch {
            LogMessage("WEB: Could not interact with YouTube page for login check")
        }
